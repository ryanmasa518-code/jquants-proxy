// api/index.js — J-Quants proxy (PROD-hardened)
// 対応:
// - 認証: Authorization ヘッダ(Bearer)のみ許可（?key / X-Proxy-Key 等の開発用バイパスは削除）
// - キャッシュTTL: /listed=24h, /prices=10m, /fins=3d
// - ログ強化: 構造化ログ(JSON)でJQエラー本文、429のリトライ回数・所要時間を記録
// - レート配慮: グローバル同時実行数を制限(デフォ5)、指数バックオフ(最大3リトライ)
// - キー管理: auth_refresh失敗時は JQ_EMAIL/PASSWORD でrefreshTokenを自動再取得(ENVセット済み前提)
...
const {
  ...
  JQ_PASSWORD,
  PROXY_BEARER,
  // 任意: 並列数とリトライの上限は環境変数でも上書き可能
  MAX_CONCURRENCY = "5",          // 銘柄ループなどの同時実行数
  MAX_RETRIES = "3",              // JQ呼び出しリトライ回数
  BASE_BACKOFF_MS = "400",        // リトライ初期待ち(ms)
} = process.env;
...
function requireProxyBearer(req) {
  if (!PROXY_BEARER) return true; // env未設定ならスキップ（必要なら強制true→falseに）
...
// ===== 構造化ログ =====
function logInfo(evt, extra={}) {
  console.log(JSON.stringify({ lvl:"info", ts: nowIso(), evt, ...extra }));
}
function logWarn(evt, extra={}) {
  console.warn(JSON.stringify({ lvl:"warn", ts: nowIso(), evt, ...extra }));
}
function logError(evt, extra={}) {
  console.error(JSON.stringify({ lvl:"error", ts: nowIso(), evt, ...extra }));
}
...
// ===== J-Quants 呼び出し (リトライ&ログ) =====
async function jqFetchRaw(url, headers) {
    ...
    while (true) {
      try {
        ...
        if (res.status === 429 && attempt < RETRIES) {
          const backoff = BACKOFF0 * Math.pow(2, attempt); // 400,800,1600...
...
async function getRefreshTokenByPassword() {
  ...
  const rt = String(data.refreshToken).trim();
...
async function ensureIdToken() {
  ...
  try {
    ...
    // フォールバック: メール/パスでrefreshToken再取得→再試行
    if (JQ_EMAIL && JQ_PASSWORD) {
...
async function handleHealth(_req, res) {
  safeJson(res, 200, {
    ok: true,
    ts: nowIso(),
    idToken_valid_ms: Math.max(0, cache.idTokenExpAt - Date.now()),
    concurrency: { limit: LIMIT, active },
  });
}
...
async function handleUniverseListed(req, res) {
  ...
  try {
    ...
    setCached(key, data, 24 * 60 * 60_000); // 24h
...
async function handlePricesDaily(req, res) {
  ...
  try {
    ...
    setCached(cacheKey, data, 10 * 60_000); // 10m
    safeJson(res, 200, data);
  } catch (e) {
    safeJson(res, 500, { error: String(e.message || e) });
  }
...
function ttmFromQuarterly(rows, fields) {
  ...
  for (const [label, candidates] of Object.entries(fields)) {
    let sum = 0;
    for (const r of recent4) {
      for (const k of candidates) { if (r[k]!=null) { sum += num(r[k]); break; } }
    }
    out[label] = sum;
  }
  return out;
...
async function handleFinsStatements(req, res) {
  ...
  try {
    ...
    setCached(cacheKey, payload, 3 * 24 * 60 * 60_000); // 3d
...
// ===== screen/liquidity（TTL/並列制限対応で据え置き）=====
...
async function handleScreenBasic(req, res) {
  ...
  try {
    ...
    // サンプル上限は300（必要に応じて拡張）。内部で withLimit が動くので同時実行は制御される。
    const sample = list.slice(0, Math.min(300, list.length));
    const out = [];
    ...
    for (const it of sample) {
      ...
      try {
        ...
        logWarn("screen_basic.row_skip", { code, err: String(e).slice(0,300) });
        continue;
      }
    }
